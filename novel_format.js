...
function pre_replace(doc) {
  return (
    doc
      ...
      // panda ads
      .replace(
        /pᴀɴᴅᴀ ɴoᴠᴇʟ|pᴀɴᴅᴀ-ɴoᴠᴇʟ|ρꪖꪕᦔꪖꪕꪫꪣꫀ​ꪶ​|ραꪒⅆα-ꪒꪫꪥꫀ​ꪶ|ραꪒⅆα ꪒꪫꪥꫀ​ꪶ​|pᴀɴᴅᴀ-ɴᴏᴠᴇʟ.ᴄᴏᴍ/gi,
        ""
      )
      // dash
      .replace(/[─]+/g, "─")
...
function exclamation(doc) {
  return (
    doc
      ...
      .replace(/Aa([^\w])/g, "Ah$1")
      .replace(
        /([^\w])(Mm-hmm|Mmmhmm|Mmhmm|mmmhmm|mmhmm|Mhm)([ |?|!|\.|,|…])/g,
        "$1Uh-huh$3"
      )
      ...
      .replace(/Nn/g, "Neun")
      .replace(/([^\w])([Pp]w*f+t+f*|[Pp]ff+t*f*)([^\w])/g, "$1Pfft$3")
      .replace(/Seug/g, "Shug")
...
function getElementWithMaxChlidenNode(tagName = "div") {
  return [...document.getElementsByTagName(tagName)].reduce(
...
function setNavigatorWithArrowKey() {
  ...
  try {
    ...
    try {
      ...
      // first chapter
      if (prev_chapter == "")
        prev_chapter = next_chapter != "" ? current_url : "";
      // latest chapter
      if (next_chapter == "")
        next_chapter = prev_chapter != "" ? current_url : "";
    }
  }
...
function countElements(element, condition) {
  return [...element.children].reduce(
    (total, e) => (condition(e) ? total + 1 : total),
    0
  );
}
function process() {
  ...
  if (
    countElements(content, (e) => e.tagName == "P") < 5 &&
    countElements(content, (e) => e.tagName == "BR") < 5
  ) {
    console.log(content);
    return;
  }
...
document.addEventListener("keydown", function (e) {
  if (e.key === "Q") {
    //TODO: type mode like vimium; use cookie?
    transfrom_mode = !transfrom_mode;
    console.log("transfrom mode: ", transfrom_mode);
  }
  if (e.key === "S" && transfrom_mode) process();
  if (e.key === "H" && transfrom_mode) full_book();
  if (e.key === "T" && transfrom_mode) reset();
